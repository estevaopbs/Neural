...
or none it will be identified by its \_\_hash\_\_ number and if it receives a string it will be identified by the 
string itself.
...
changed by a mutate call. When it's True, a random bias will be generated. It can also receives a int or float tohave 
its value pre-set;
...
### Functions
**load_data**: This method reads a .json document and returns the neural networks saved in it. It must be called this 
way:
```py
